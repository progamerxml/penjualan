	...
	<!-- <div class="col-sm pb-3">
		<div class="d-flex flex-column py-2 bg-warning px-3 rounded">
			<div class="col-sm text-white fw-bolder fs-1">          			
				<strong>
					<?php 
						$query_qty = mysql_query("SELECT SUM( lastbalance ) as lbc FROM mutasi_kas");
						$jumlah = mysql_fetch_object($query_qty);
						echo "Rp. ".number_format($jumlah->lbc,2,",",".");
					?>
          		</strong>
			</div>
			<div class="py-1 col-sm text-white">
			...
			<div class="col-sm p-1 text-sm-end">
          		<a href="<?php echo'?h=lastbalance'; ?>" class=" border rounded px-1 text-decoration-none text-white float-end">lihat data</a>
			</div>
		</div>
	</div>
	...
	<div class="col-sm pb-3">
		<div class="d-flex flex-column py-2 bg-warning px-3 rounded">
			<div class="col-sm text-white fw-bolder fs-1">          			
				<strong>
					<?php 
						$query_cbc = mysql_query("SELECT SUM( currbalance ) as cbc FROM mutasi_kas");
						$jmlh = mysql_fetch_object($query_cbc);
						echo "Rp. ".number_format($jmlh->cbc,2,",",".");
					?>
          		</strong>
			</div>
			<div class="py-1 col-sm text-white">
			...
			<div class="col-sm p-1 text-sm-end">
          		<a href="<?php echo'?h=currbalance'; ?>" class=" border rounded px-1 text-decoration-none text-white float-end">lihat data</a>
			</div>
		</div>
	</div> -->
</div>
...
$query = "SELECT * FROM   mutasi_kas  where id>0 $where order by id desc limit  $from, $jml "; 
 
$rs = mysql_query($query); 

	while ($r = mysql_fetch_object($rs)) { 
		$lastbalace ="Rp. ".number_format($r->lastbalance,2,",","."); $currbalance ="Rp. ".number_format($r->currbalance,2,",",".");
		echo "<tr valign='top'> 
		<td>$r->id</td>
		<td>$r->kas_id</td>  
		<td>$r->debet</td>  
