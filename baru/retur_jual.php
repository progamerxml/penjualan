...
function CEKINPUTRETUR(i,j){ 
	...
	if (sisa_stok_retur-qty_tmp_retur<0) {
		...
		return false;
		 ...
		 window.location="?h=retur_jual&noretur="+noretur+"&sisa_stok_retur="+sisa_stok_retur+"&kode_cari="+kode_cari+"&nama_cari="+nama_cari+"&Cari="+Cari+"&qty_tmp_retur="+qty_tmp_retur+"&hjual_tmp_retur="+hjual_tmp_retur+"&id_jual="+id_jual+"&addtmpretur=OK"; 
		return false;	
	} 
...
if($_POST['Simpan_Retur_Jual']=="Simpan"){ 
 ...
 $sales=$_POST[sales_h]; 
	...
	while ($r=mysql_fetch_object($query)){  
		...
		//insert  retur rinci
		mysql_query("insert into retur_jual_rinci (noretur,nofaktur,sales,produk_id,harga_jual,qty) value ('".$r->noretur."','".$r->nofaktur."','".$r->sales."','".$r->produk_id."','".$r->harga_jual."','".$r->qty."')"); 
