      ...
      <div class="row mb-3">
        <div class="col-sm-12 rounded">
          <div class="d-flex flex-row bg-info">
            <input type="text" name="keyword" placeholder="masukan kode / nama produk" class="border border-2 rounded-start border-warning py-1 px-2 flex-fill" value="<?php echo $_GET[keyword]; ?>" id="keyword_cari">
...
$q = "select count(*) as jml from produk where jml_stok<=1000 AND jml_stok > 100";
$rsjml = mysql_query($q);
$rjml = mysql_fetch_object($rsjml);
$rCount = $rjml->jml;


$query = "SELECT * FROM   produk  where jml_stok<=1000 AND jml_stok > 100 $where limit  $from, $jml "; 
