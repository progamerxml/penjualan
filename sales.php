...
if($_POST[Simpan_sales]=="Simpan"){  
	$sql="insert into sales (nama,alamat,keterangan) value ('".$_POST[nama]."','".$_POST[alamat]."','".$_POST[keterangan]."')";   
	$act="Tambah";
} 

if($_POST[Simpan_sales]=="Ubah"){  
	$sql="Update sales set nama='".$_POST[nama]."',alamat='".$_POST[alamat]."',keterangan='".$_POST[keterangan]."' where id='".$_POST[id]."'";   
	$act="Ubah";
} 
...
if ($_GET[act]=="hapkat"){
	if (mysql_query("delete from  sales where id='".$_GET[id]."' ")){
		$res="Berhsil hapus sales";	
	}else{
		$res="Gagal hapus sales --> ".mysql_error();	
	}
	echo "<script language='javascript'> window.location='?h=$_GET[h]&res=$res'</script>";  
}

if ($_GET[act]=="editkat"){
	 $set=mysql_fetch_assoc(mysql_query("select nama,alamat,keterangan  from sales where id='$_GET[id]'"));
	 $btn="Ubah";
}
...
<div class="form-style-2-heading">Input sales</div>
...
<label><span> </span><input type="submit" value="<?php echo $btn;?>" name="Simpan_sales" /> <input type="button" value="Batal" name="Batal" onclick="window.location='?h=<?php echo $_GET[h];?>'" />  </label>
</form>
</div> 
<table id="tabel">
  <tr>
