...
<?php 
	...
	$where="";
	if ($keyword!=""){ $where.=" and  retur_distribusi.noretur like '%$keyword%' or retur_distribusi.produk_id like '%$keyword%'"; }

    $q = "select count(*) as jml from retur_distribusi where 1  $where";  
    $rsjml = mysql_query($q);
    $rjml = mysql_fetch_object($rsjml);
    $rCount = $rjml->jml;

	$query = "SELECT retur_distribusi.produk_id, retur_distribusi.noretur, retur_distribusi.no_distribusi, retur_distribusi.sales, retur_distribusi.admin, retur_distribusi.qty, retur_distribusi.total, retur_distribusi.tgl_retur, produk.nama FROM retur_distribusi INNER JOIN produk ON retur_distribusi.produk_id = produk.produk_id WHERE retur_distribusi.id>0 $where order by retur_distribusi.tgl_retur desc limit $from, $jml "; 
?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"> 
	<tr>
    	...
    	<div class="form-style-2-heading">Laporan Retur Distribusi</div>
	...
	<div class="row pb-2">
		<div class="col-sm pb-3">
			<div class="d-flex flex-column py-2 bg-warning px-3 rounded">
				<div class="col-sm text-white fw-bolder fs-1">
					<?php
						$query_qty = mysql_query("SELECT SUM( qty ) as jml FROM retur_distribusi");
						$jumlah = mysql_fetch_object($query_qty);
						echo $jumlah->jml;
					?>
				</div>
				<div class="py-1 col-sm fw-bold text-white">
					total retur Distribusi
				</div>
			</div>
		</div>
		...
		<div class="col-sm pb-3">
			<div class="d-flex flex-column py-2 bg-warning px-3 rounded">
				<div class="col-sm text-white fw-bolder fs-1">
					<?php
						$query_qty = mysql_query("SELECT SUM( total ) as uang FROM retur_distribusi");
						$jumlah = mysql_fetch_object($query_qty);
						echo "Rp. ".number_format($jumlah->uang,2,",",".");
					?>
				</div>
				<div class="py-1 col-sm fw-bold text-white">
					total uang retur Distribusi
				</div>
			</div>
		</div>
	</div>
...
<table id="tabel" style="margin-top:3px;">
	...
	<?php
		...
		"<tr valign='top'> 
			...
			<td>$r->sales</td> 
			<td>$r->admin</td> 
			<td>No Distribusi : $r->no_distribusi, <br> waktu : $r->tgl_retur</td>
		</tr>  ";
		$i++;
		}
